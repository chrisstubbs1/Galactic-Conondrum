...
public class Enemy : MonoBehaviour
{
    ...
    public virtual void Move()
    {
        ...
        {
            Vector3 tempPos = pos;
            if (tempPos.x >= 110 || tempPos.x <= -110)
            {

                tempPos.y -= escapespeed * Time.deltaTime;
                pos = tempPos;
            ...
            else
            {
                if (tempPos.x <= 0)
                {
                    tempPos.x -= escapespeed * Time.deltaTime;
                }
                else
                {
                    tempPos.x -= -escapespeed * Time.deltaTime;
